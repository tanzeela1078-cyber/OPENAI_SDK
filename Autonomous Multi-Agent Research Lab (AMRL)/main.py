...
import asyncio
from openai import BaseModel, RateLimitError, InternalServerError
...
wiki = wikipediaapi.Wikipedia(
    language='en', 
    user_agent='ResearchAgent/1.0 (https://openai.com; contact: research@example.com)'
)

@lru_cache(maxsize=100)
def get_wikipedia_summary(query: str, sentences: int = 3) -> str:
    """
    Fetch a concise summary from Wikipedia for a given query.
    Caches results to reduce repeated lookups.
    """
...
def get_wikipedia_page_info(query: str) -> Dict[str, Any]:
    """
    Return structured information about a Wikipedia page:
    - title
    - summary
    - fullurl
    - categories
    ...
    return {
        "title": page.title,
        "summary": page.summary[:1000] + "...",  # limit for brevity
        "url": page.fullurl,
...
def search_wikipedia(query: str, sentences: int = 3) -> str:
    ...
    Uses cached Wikipedia API to prevent API overload.
    """
    print(f"\n🔍 [Tool Triggered] search_wikipedia() for: {query}")
    
    try:
        return get_wikipedia_summary(query, sentences)
    except Exception as e:
...
def wikipedia_page_info(query: str) -> str:
    ...
    Args:
        query: The topic to get information about
    """
    try:
        info = get_wikipedia_page_info(query)
        if "error" in info:
            return info["error"]
        ...
        result += f"Categories: {', '.join(info['categories'][:5])}\n"
        return result
    except Exception as e:
        return f"Error getting Wikipedia page info: {str(e)}"
...
def generate_pdf_from_markdown(markdown_content: str, output_filename: str = "research_paper.pdf") -> str:
    ...
    try:
        ...
        with open(output_filename.replace('.pdf', '.txt'), 'w', encoding='utf-8') as f:
            f.write(markdown_content)
        return output_filename.replace('.pdf', '.txt')
...
ResearchAgent = Agent(
    name="ResearchAgent",
    instructions=(
        "You are an advanced research agent. Your task is to:\n"
        "1. Use search_wikipedia tool 2-3 times to find key information about the research topic\n"
        "2. For cross-domain topics, use search_arxiv, search_ieee, or search_pubmed as appropriate\n"
        "3. Use detect_contradictions to identify conflicting information\n"
        "4. Use dynamic_context_summarizer if content becomes too long\n"
        "5. Summarize your findings in 4-5 detailed paragraphs (200+ words)\n"
        "6. MUST hand off to AnalystAgent when research is complete\n"
        "IMPORTANT: Be efficient - use appropriate tools for the domain, detect contradictions, and summarize effectively."
    ),
    tools=[search_wikipedia, wikipedia_page_info, search_arxiv, search_ieee, search_pubmed, detect_contradictions, dynamic_context_summarizer, store_agent_result],
